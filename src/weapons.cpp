...
Weapons::Weapons() : m_scriptInterface("Weapon Interface")
{
...
{
	return static_cast<int32_t>(std::ceil((attackSkill * (attackValue * 0.05)) +
		(attackValue * 0.5)));
}
...
bool Weapon::configureEvent(const pugi::xml_node& node)
{
	...
	for(pugi::xml_node vocationNode = node.first_child();
		vocationNode; vocationNode = vocationNode.next_sibling())
	{
...
int32_t Weapon::playerWeaponCheck(Player* player, Creature* target) const
{
	...
	if(std::max<uint32_t>(Position::getDistanceX(playerPos, targetPos), 
		Position::getDistanceY(playerPos, targetPos)) > trueRange)
		return 0;
...
//This function will always return true (should be void)
...
void Weapon::onUsedAmmo(Item* item, Tile* destTile) const
{
	...
	if(ammoAction == AMMOACTION_REMOVECOUNT)
	{
		...
		g_game.internalMoveItem(item->getParent(), destTile, 
			INDEX_WHEREEVER, item, 1, nullptr, FLAG_NOLIMIT);
...
{
	skillpoint = player->getAddAttackSkill() && player->getLastAttackBlockType() != BLOCK_IMMUNITY;
	...
	else
	{
		...
		if(!Position::areInRange<1, 1, 0>(player->getPosition(), target->getPosition()))
		{
			...
			for(const auto& dir : destList)
			{
				...
				if(tmpTile && !tmpTile->hasProperty(CONST_PROP_IMMOVABLEBLOCKSOLID) && 
					tmpTile->ground != nullptr)
				{
					destTile = tmpTile;
					break;
				}
